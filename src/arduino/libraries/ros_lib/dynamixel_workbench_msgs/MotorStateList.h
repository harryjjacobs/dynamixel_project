#ifndef _ROS_dynamixel_workbench_msgs_MotorStateList_h
#define _ROS_dynamixel_workbench_msgs_MotorStateList_h
...
namespace dynamixel_workbench_msgs
{
  ...
  class MotorStateList : public ros::Msg
  {
    public:
      ...
      typedef dynamixel_workbench_msgs::MotorState _motor_states_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(motor_states_lengthT > motor_states_length)
        this->motor_states = (dynamixel_workbench_msgs::MotorState*)realloc(this->motor_states, motor_states_lengthT * sizeof(dynamixel_workbench_msgs::MotorState));
      motor_states_length = motor_states_lengthT;
      for( uint32_t i = 0; i < motor_states_length; i++){
      offset += this->st_motor_states.deserialize(inbuffer + offset);
        memcpy( &(this->motor_states[i]), &(this->st_motor_states), sizeof(dynamixel_workbench_msgs::MotorState));
      }
     return offset;
    }

    const char * getType(){ return "dynamixel_workbench_msgs/MotorStateList"; };
    const char * getMD5(){ return "147fda78bebbc05d0095969e9a706898"; };
